...
The script generated 27 out of 200 requested questions because it removes near-duplicates using embedding similarity (threshold 0.85). `data_loader.py` is also much simpler now. The seed questions are in the repo: `additional_questions.txt`. The goal for this support bot isn’t perfect responses. It’s to create enough realistic variation to test retrieval quality, duplicate filtering, and edge cases.
